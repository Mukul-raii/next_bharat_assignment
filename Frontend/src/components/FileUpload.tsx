...
const FileUpload: React.FC<FileUploadProps> = ({ onUploadSuccess }) => {
  ...
  const { uploadDocument, uploading, error, success, resetMessages } =
    useUpload();
  ...
  return (
    <div className="bg-white p-6">
      ...
      <div
        className={`border border-dashed border-gray-300 rounded p-10 px-[30px] text-center cursor-pointer transition-all duration-200 bg-gray-50 mb-4 hover:border-gray-500 hover:bg-gray-100 ${
          dragActive ? "border-gray-800 bg-gray-200" : ""
        }`}
        ...
        <label htmlFor="file-input" className="cursor-pointer block">
          {file ? (
            <div className="flex items-center gap-[15px] justify-center">
              ...
              <div>
                ...
                <div className="text-gray-400 text-xs mt-1">
                  {DocumentController.formatFileSize(file.size)}
                </div>
              </div>
            </div>
          ) : (
            <div className="text-gray-500">
              ...
              <p className="my-2.5 text-sm text-gray-800 font-normal">
                Drag & drop or click to select
              </p>
              <small className="text-gray-400 text-xs">
                PDF, JPG, PNG, DOCX (max 100MB)
              </small>
            </div>
          )}
        </label>
      ...
      {success && (
        <div className="mt-4 p-3 rounded text-sm border bg-gray-100 text-gray-800 border-gray-300">
          ✅ {success}
        </div>
      )}

      {error && (
        <div className="mt-4 p-3 rounded text-sm border bg-gray-100 text-gray-900 border-gray-400">
          ❌ {error}
        </div>
      )}
    </div>
